...
import { $createTextNode, $getNodeByKey, $getSelection, $isRangeSelection, $isTextNode, $setSelection, BaseSelection, COMMAND_PRIORITY_LOW, KEY_ARROW_RIGHT_COMMAND, KEY_TAB_COMMAND, NodeKey } from 'lexical';
import { useCallback, useEffect } from 'react';
import { addSwipeRightListener } from '../../util/swipe';
...
export const autoCompleteUuid = Math.random().toString(36).replace(/[^a-z]+/g, '').substring(0, 6);

const $search = (selection: null | BaseSelection): [boolean, string] => {
...
export const AutocompletePlugin = () => {
  ...
  useEffect(() => {
    ...
    const updateAsyncSuggestion = (refSearchPromise: SearchPromise, newSuggestion: null | string) => {
      if (searchPromise !== refSearchPromise || newSuggestion === null) return; // Outdated or no suggestion

      editor.update(
        () => {
          const selection = $getSelection();
          const [hasMatch, match] = $search(selection);
          if (!hasMatch || match !== lastMatch || !$isRangeSelection(selection)) return; // Outdated

          const selectionCopy = selection.clone();
          const node = $createAutocompleteNode(autoCompleteUuid);
    ...
    const $handleAutocompleteNodeTransform = (node: AutocompleteNode) => {
      ...
      if (node.__uuid === autoCompleteUuid && key !== autocompleteNodeKey) {
    ...
    const $handleAutocompleteIntent = () => {
    ...
    const unmountSuggestion = () => editor.update($clearSuggestion);
