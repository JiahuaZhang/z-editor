...
BEGIN
    ...
    IF TRIM(pgroonga_query_string) = '' THEN
        SELECT ARRAY_AGG(id) INTO result_ids FROM editor_documents;
        RETURN COALESCE(result_ids, ARRAY[]::UUID[]);
    END IF;

    RAISE NOTICE 'PGroonga query: %', TRIM(pgroonga_query_string);

    SELECT ARRAY_AGG(id) INTO result_ids
    FROM editor_documents
    WHERE content &@~ TRIM(pgroonga_query_string);
